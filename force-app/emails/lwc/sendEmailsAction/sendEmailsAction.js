...
export default class SendEmailsAction extends LightningElement {
    ...
    wiredRecipientInfo(recipients) {
        if (recipients.data) {
            ...
            let messages = ["Unable to retrieve recipient information."];
    ...
    wiredFromAddressOptions(result) {
        if (result.data) {
            ...
            let messages = ["Unable to retrieve originator options."];
        ...
        sendEmail({emailRequest})
            .then((result) => {
                if (!!result.errors) {
                    ...
                    // This component isn't expected to be the actual action
                    // component, so in order to close the modal from this depth we
                    // configure the CloseActionScreenEvent to be able to bubble
                    ...
                    this.dispatchEvent(new ShowToastEvent({
                        ...
                        message: `Successfully sent ${targetObjectIds.length} emails.`,
                        variant: "success"
                    }));
                }
            })
            .catch((error) => {
                let messages = ["Unable to send email."];
