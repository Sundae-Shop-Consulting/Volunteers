...
export default class SendEmailsForm extends LightningElement {
    ...
    get recipientsHelpMessage() {
        ...
        if (this.isRecipientsFiltered) {
            messages.push('Some invalid recipients were removed.');
        }

        if (Array.isArray(this.recipients) && this.recipients.length == 0) {
            messages.push('Recipients are required.');
        }

        if (this.isRecipientsInvalidEmail) {
            messages.push(
                'Some recipients do not have a valid email address.  Please ' +
    ...
    */
    ...
    get isBodyValid() {
        if (!this.htmlBody) {
            this.bodyErrorMessage = 'Complete this field.';
            return false;
        } else {
    ...
    get hasRelatedToIcon() {
        return !!this.relatedTo?.colorStyle && !!this.relatedTo?.iconUrl
    }
